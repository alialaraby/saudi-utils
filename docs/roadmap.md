...
- Normalize only ASCII spaces/case without checking IBAN validity; format only valid canonical input.
...
- Normalize only documented unseparated representations to the discriminated union; let validators reject invalid prefixes and lengths.
...
- Component validators/wrappers and narrow Short Address normalizer; normalization does not enforce the four-digit rule.
